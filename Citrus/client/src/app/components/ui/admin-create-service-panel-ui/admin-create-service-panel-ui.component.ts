	...
	getErrorMessage(inputName: string) {
		switch (inputName) {
			case 'service': {
				...
				return this.createServiceForm.controls['service'].hasError('pattern')
			...
			case 'duration': {
				...
				return this.createServiceForm.controls['duration'].hasError('pattern')
					? "Введите длительность процедуры в таком виде: '1:30' "
					: '';
			}
			default:
				return 'Ошибка';
