...
export class AdminOrderListPanelWrapperComponent implements OnDestroy {
	...
	getOrdersData(pageSize: number, startItem: number): void {
		...
		this.subscriptions.push(
			this.http.getOrdersData(pageSize, startItem).subscribe(data => {
				if (data.length > 0) {
	...
	onButtonClick(e: { action: string; orderId: number }): void {
		switch (e.action) {
			case 'cancel':
				this.subscriptions.push(
					this.http.cancelOrder(e.orderId).subscribe({
						next: response => {
			...
			case 'done':
				this.subscriptions.push(
					this.http.completeOrder(e.orderId).subscribe({
						next: response => {
							if (response.statusCode === 0) {
								this.getOrdersData(this.pageSize, this.startItem);
							}
